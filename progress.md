...
- 2026-03-16: `p2/index.html` 改为实验版本输入页，支持输入 1-4 并跳转到 `/?variant=p2&condition=<code>`；`js/main.js` 新增 `condition/combo/group` 参数解析，可强制指定 AI/人类新闻与 AI/非AI 游戏的四种组合，不再走随机/平衡分配。
- 2026-03-16: 应用户要求将实验编号入口并回主界面：`index.html` 开场第一步新增“实验编号”输入，`js/main.js` 优先读取首页输入的 1-4 编号决定四种实验组合；参与者现在直接访问根域名即可进入盲测流程，无需 `/p2/` 子路径。
- 2026-03-16: 按最新需求移除前测、后测与激励链路：`index.html` 删除前测步骤、后测弹窗和奖励弹窗；`js/main.js` 改为输入实验编号和姓名后直接开始；`js/ui.js` 结局提交不再弹问卷或兑换页，奖励信息固定为空。
